...
import { Button, FormField, Toggle, Badge } from '@must-iq-web/components/ui';
...
export default function ProfilePage() {
    ...
    const handleUpdateProfile = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Merge updated user data into store
    ...
    const goBack = () => {
        router.push(fromAdmin ? '/chat?from=admin' : '/chat');
    };

    return (
        <div className="flex-1 overflow-y-auto p-8 flex justify-center items-start pt-16" style={{ background: 'var(--bg)', color: 'var(--ink)' }}>
            <div className="w-full max-w-2xl">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h1 className="text-2xl font-['DM_Serif_Display'] mb-1">User Profile</h1>
                        <p className="text-[13px] text-[var(--muted)]">Manage your account details and password.</p>
                    </div>
                    <Button variant="ghost" onClick={goBack} style={{ fontSize: 13 }}>
                        ← Back to {fromAdmin ? 'Admin' : 'Chat'}
                    </Button>
                </div>

                {/* Profile Details */}
                <section className="p-6 rounded-2xl mb-6 shadow-sm border" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
                    ...
                    <form onSubmit={handleUpdateProfile} className="flex flex-col gap-6" noValidate>
                        ...
                        <div className="flex gap-4">
                            <div className="flex-1 flex flex-col gap-1.5 opacity-60">
                                <label className="text-[11.5px] font-semibold tracking-[0.05em] uppercase ml-0.5" style={{ color: 'var(--muted)' }}>Role</label>
                                <FormField label="" value={user?.role || ''} readOnly />
                            </div>
